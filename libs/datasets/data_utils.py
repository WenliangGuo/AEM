...
def truncate_feats(
    ...
    # truncate_data_dict = partial_deepcopy(data_dict, exclude_keys=['vlm_info'])
    truncate_data_dict = copy.deepcopy(data_dict)
    ...
    for _ in range(max_num_trials):
        ...
        left = torch.maximum(window[:, 0], truncate_data_dict['segments'][:, 0])
        right = torch.minimum(window[:, 1], truncate_data_dict['segments'][:, 1])
        inter = (right - left).clamp(min=0)
        area_segs = torch.abs(
            truncate_data_dict['segments'][:, 1] - truncate_data_dict['segments'][:, 0])
    ...
    st = int(left[seg_idx].min().item())
    ed = int(right[seg_idx].max().item())

    # feats: C x T
    truncate_data_dict['feats'] = truncate_data_dict['feats'][:, st:ed].clone()
    # segments: N x 2 in feature grids
    truncate_data_dict['segments'] = torch.stack((left[seg_idx], right[seg_idx]), dim=1)
    # shift the time stamps due to truncation
    truncate_data_dict['segments'] = truncate_data_dict['segments'] - st
    # action labels: N
    ...
    if 'bbox_class' in data_dict:
        truncate_data_dict['bbox_class'] = truncate_data_dict['bbox_class'][st:ed]
    # bbox class: T x num_node x 4
    if 'bbox' in data_dict:
        truncate_data_dict['bbox'] = truncate_data_dict['bbox'][st:ed, :]
    # edge map: T x num_node x num_node
    if 'edge_map' in data_dict:
        truncate_data_dict['edge_map'] = truncate_data_dict['edge_map'][st:ed, :, :]

    truncate_data_dict['start'] = st
    truncate_data_dict['end'] = ed

    return truncate_data_dict
